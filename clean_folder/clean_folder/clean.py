...
cmf = {} # count moving files - потрібен для перейменування файлів з однаковими іменами, для кожної окремої папки буде починатися свій відлік,
...
def moving_files(obj:Path, folder_path:Path, new_folder:str):
        ...
        way2 = dest_folder.joinpath(str(cmf.get(new_folder)) + normalize_z) # поробив змінні щоб кожного разу не виконувало одну і ту ж функцію 
...
def main():  
    if len(sys.argv) > 1:  # визначення шляху до папки
    ...
    for obj in usi_objekty: # сортування
...
# не було повторного сортування, а також не лізло в папку з розпакованими архівами                        
    ...
    for key in all_known_ext:           # створення словника з відомими розширеннями
